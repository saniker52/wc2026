...
router.get('/report/pdf', (req, res) => {
  ...
  const now = new Date();
  const dateStr = now.toLocaleString('en-GB', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit', timeZone:'Asia/Kuwait' }) + ' (Kuwait Time)';
  ...
  doc.fill('#ffffff').fontSize(18).font('Helvetica-Bold')
     .text('CHALET 1267 — WC 2026 Leaderboard', 50, 18, { align: 'center', width: doc.page.width - 100 });
  doc.fill('#aaaaaa').fontSize(8)
     .text(`Generated: ${dateStr}`, 50, 46, { align: 'center', width: doc.page.width - 100 });

  doc.y = 90;

  // ── Leaderboard Table ───────────────────────────────────────────────────────
  doc.fill(DARK).fontSize(11).font('Helvetica-Bold').text('Leaderboard', 50, doc.y);
  doc.moveDown(0.3);
  ...
  if (prizees.length > 0) {
    doc.moveDown(0.8);
    doc.fill(DARK).fontSize(11).font('Helvetica-Bold').text('Prize Payouts', 50, doc.y);
    doc.moveDown(0.3);
